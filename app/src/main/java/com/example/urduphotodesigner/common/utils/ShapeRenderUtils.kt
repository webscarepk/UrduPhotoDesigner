...
object ShapeRenderUtils {
    ...
    fun buildShapePath(shapeType: ShapeType, rect: RectF, cornerRadius: Float = 5f): Path {
        val cx = rect.centerX()
        val cy = rect.centerY()
        val path = Path()
        ...
        when (shapeType) {
            ...
            ShapeType.ROUNDED_RECTANGLE -> path.addRoundRect(rect, cornerRadius, cornerRadius, Path.Direction.CW)
            ShapeType.ELLIPSE -> path.addOval(rect, Path.Direction.CW)
            ShapeType.LINE -> {
            ...
            ShapeType.HEART -> {
                ...
                path.cubicTo(
                    rect.left, rect.bottom - curveRadius, rect.left, rect.top, cx, rect.top + curveRadius / 2f
                )
                path.cubicTo(rect.right, rect.top, rect.right, rect.bottom - curveRadius, cx, rect.bottom)
            }
