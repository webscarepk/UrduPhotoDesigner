...
class PreferencesFragment : Fragment() {
    ...
    private fun setupRecycler(rv: RecyclerView, type: ExportViewType) {
        ...
        rv.adapter = ExportOptionAdapter(
            items,
            type,
            displayMode = false   // ✅ compact mode
        ) { selected ->
            when (selected) {
                is ExportResolution -> viewModel.updateExportOptionsAndSave(
                    viewModel.exportOptions.value!!.copy(resolution = selected)
                )
                is ExportQuality -> viewModel.updateExportOptionsAndSave(
                    viewModel.exportOptions.value!!.copy(quality = selected)
                )
                is ExportFormat -> viewModel.updateExportOptionsAndSave(
