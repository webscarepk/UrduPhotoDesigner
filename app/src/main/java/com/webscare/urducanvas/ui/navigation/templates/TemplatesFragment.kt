...
//    }
    ...
    private fun filterTemplates(
        ...
        val bySub = if (subcategory.equals("All", true)) {
            ...
            source.filter { it.subcategory?.trim().orEmpty().equals(subcategory, true) }
        }
                    ...
                    when (state) {
                        is TemplateDownloadState.Progress -> {
                            if (isGridMode()) {
                                ...
                                subcategoryAdapter.updateTemplateProgress(state.template.id, state.progress, true, false)
                            }
                        }
    ...
    private fun currentVisibleCount(): Int = if (isGridMode()) {
        templatesAdapter.itemCount
    } else {
        subcategoryAdapter.itemCount
    }
