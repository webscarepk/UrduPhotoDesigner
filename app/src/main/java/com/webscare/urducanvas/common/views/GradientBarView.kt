 ...
 */
class GradientBarView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : android.view.View(context, attrs, defStyle) {
    ...
    private fun xToPos(x: Float) = ((x - trackStart) / trackWidth).coerceIn(0f, 1f)
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (dragging && activeHandle >= 0) {
                    ...
                    val lower = if (activeHandle > 0) {
                        positions[activeHandle - 1] + 0.001f
                    } else {
                        0f
                    }
